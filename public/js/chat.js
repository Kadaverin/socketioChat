(function () {
  ...
  function handlelogin(){
    ...
    socket.emit('login request' , credentials)
  }
  ...
  function handleRegister(){
    ...
    socket.emit('register request' , userInfo)
  }
  ...
  function handleAuthSuccess(userObj){
    currentUser = userObj
    // показать чат, скрыть форму, запросить данные по юзерам и сообщениям
  }
  ...
  function requestForChatData(){
    socket.emit('get chat gata') // на сервере отдать юзером и сто сообщений
  }

  socket.on('get chat data success' , data => {
    // сохранить в переменные и отрендерить в дом data.users , data.msgs
  })

  socket.on('new user registered' , user => {
   // отрендерить юзера в юзер-лист
  })

  socket.on('user came', user => {
    // поменять статус 
  })
